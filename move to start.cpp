// move to start.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
public:
    ...
    Node* move_to_start(Node* vertice, int left, int right) {
        auto t2 = split(vertice, right);
        auto t1 = split(t2.first, left - 1);
        auto t12 = merge(std::make_pair(t1.second, t1.first));
...
int main()
{
    ...
    std::cin >> n >> m;
    for (int i = 0; i < n; ++i) {
        dec_t.root = dec_t.insert(dec_t.root, i, i + 1, rand());
    }
    int l, r;
    for (int i = 0; i < m; ++i) {
        std::cin >> l >> r;
        dec_t.root = dec_t.move_to_start(dec_t.root, l, r);
    }
