// k_max.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
public:
    ...
    int k_max(Node* vertice, int k) {
        if (get_count(vertice->right) == k - 1) {
            return vertice->key;
        }
        else if (get_count(vertice->right) >= k) {
            return k_max(vertice->right, k);
        }
        else if (get_count(vertice->right) < k - 1) {
            return k_max(vertice->left, k - get_count(vertice->right) - 1);
        }
    }
};
...
int main()
{
    ...
    while (n > 0) {
        ...
        if (command == 1) {
            ...
            std::cout << avl_t.k_max(avl_t.root, x) << '\n';
        }
        --n;
    }
}
